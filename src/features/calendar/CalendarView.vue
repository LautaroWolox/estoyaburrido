...
function goToday(){const today=new Date(store.now);month.value=startOfMonth(today);selected.value=today}
...
<div class="calendar-layout"><Card class="content-card"><template #title><div class="calendar-toolbar"><div><Button icon="pi pi-chevron-left" text rounded @click="month=subMonths(month,1)"/><Button label="Hoy" severity="secondary" text @click="goToday"/><Button icon="pi pi-chevron-right" text rounded @click="month=addMonths(month,1)"/></div><strong>{{format(month,'MMMM yyyy',{locale:es})}}</strong></div></template><template #content><div class="calendar-labels"><span v-for="label in labels" :key="label">{{label}}</span></div><div class="calendar-grid"><button v-for="day in days" :key="day.toISOString()" :class="['calendar-day',{muted:day.getMonth()!==month.getMonth(),selected:isSameDay(day,selected),today:isSameDay(day,store.now)}]" @click="selectDay(day)"><b>{{format(day,'d')}}</b><span><i v-if="hasMedication(day)" class="marker medication"/><i v-if="hasRoutine(day)" class="marker routine"/><i v-for="event in eventsFor(day).slice(0,3)" :key="event.id" class="marker" :style="{background:event.color}"/></span><small v-if="eventsFor(day).length">{{eventsFor(day).length}}</small></button></div><div class="legend"><span><i class="marker medication"/> Medicación</span><span><i class="marker routine"/> Rutinas</span><span><i class="marker"/> Eventos</span></div></template></Card>
